...
def load_tokenizer(model_args: "ModelArguments") -> "TokenizerModule":
    ...
    # Check if using Tiktoken tokenizer
    use_tiktoken = getattr(model_args, 'use_tiktoken', False)
    
    if use_tiktoken:
        # Load Tiktoken tokenizer
        from ..data.tiktoken_utils import TiktokenTokenizer
        
        encoding_name = getattr(model_args, 'tiktoken_encoding', 'cl100k_base')
        logger.info_rank0(f"Loading Tiktoken tokenizer with encoding: {encoding_name}")
        ...
        # Load standard HuggingFace tokenizer
        try:
